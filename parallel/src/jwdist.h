...
std::vector<std::string> jw_outer(std::vector<std::string_view> sl1, std::vector<std::string_view> sl2){
  ...
  for(int i = 0; i < n1 ; i++){
    for(int j = 0; j < n2 ; j++){
      dis = edit_distance::jaro_winkler(sl1[i],sl2[j]);
      in+=std::to_string(dis)+",";
    }
    out.push_back(in);
    in = "";
  }
